...
class ArticleTagService:
    ...
    def attach(self, *, article_id: int, tag_id: int) -> ArticleTag:
        ...
        existing = self.repo.get(article_id, tag_id)
        if existing:
            raise HTTPException(
        ...
        self.repo.add(article_tag)
        return article_tag

    def remove(self, *, article_id: int, tag_id: int) -> None:
        article_tag = self.repo.get(article_id, tag_id)
        if not article_tag:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Tag is not attached to the article"
            )
